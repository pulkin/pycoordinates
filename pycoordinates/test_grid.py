...
class TetrahedronDensityTest(TestCase):
    @classmethod
    def setUpClass(cls):
        N = 50
        ...
        cls.grid = Grid(Basis.orthorhombic((1, 1, 1)), (x, y, z), data)
    ...
    def test_td_2(self):
        d = self.grid.tetrahedron_density((-.1, 0, .1, .2), weights=np.ones_like(self.grid.values) * .5)
        testing.assert_allclose(d, (0, 0, np.pi * 0.1, np.pi * 0.2), rtol=1e-2)

    def test_td_fail_0(self):
        g = self.grid.copy(
            vectors=self.grid.vectors[:2, :2],
            coordinates=self.grid.coordinates[:2],
            values=self.grid.values[:, :, 0, ...],
        )
        with pytest.raises(ValueError):
